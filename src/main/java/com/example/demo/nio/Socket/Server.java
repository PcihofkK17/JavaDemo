...
public class Server
{
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        // 绑定 host 地址，并指定 5 最大的进入连接数量
        server.socket().bind(new InetSocketAddress("127.0.0.1", 9999), 5);
...
class Daemon implements Runnable
{
    ...
    public void run()
    {
        while (true) {
            try {
                // Wait for an event，等待客户端访问，不然 就一直阻塞
                selector.select();
                ...
                while (it.hasNext()) {
                    ...
                    if (selKey.isAcceptable()) {
                        ...
                        // Accept the connection request. 有客户端连接的话，就会用这个方法获取到连接的 SocketChannel
...
class ServerHandler implements Runnable
{
    ...
    private void read(byte[] cache, byte[] reply) throws IOException
    {
        ...
        if (!ExtArrays.partialEquals(md5, 0, cache, 240, 16)) {
            // 与后16字节比较
            reply[0] = '?';
